  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm">
        <div className="container px-4 py-4 mx-auto d-flex justify-content-between align-items-center">
            ...
            <div className="flex items-center">
                <span className="mr-4 text-gray-600">user@example.com</span>
      ...
      <main className="container p-4 mx-auto">
        ...
        <section>
            <h3 className="mb-4 text-2xl font-semibold text-gray-600">My Documents</h3>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
               {myDocuments.map((doc) => (
                    <div key={doc.id} className="p-4 bg-white rounded-lg shadow hover:shadow-lg transition-shadow">
                       <Link href={`/doc/${doc.id}`} className="text-lg font-bold text-indigo-700">{doc.title}
                       </Link>
                       <p className="mt-2 text-sm text-gray-500">Last opened: {doc.lastOpened}</p>
                       <div className="mt-4 text-right">
                          <button onClick={() => deleteDocument(doc.id)} className="text-sm font-medium text-red-500 hover:text-red-700">
                            Delete
                          </button>
                       </div>
                    </div>
        ...
        <section className="mt-12">
            <h3 className="mb-4 text-2xl font-semibold text-gray-600">Shared With Me</h3>
             <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
